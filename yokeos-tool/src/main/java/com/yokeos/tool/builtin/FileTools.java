 ...
 *
 * <p>硬规矩：每个方法第一件事是 Sandbox 检查位（24 节接线）——校验不过文件根本不碰。超长截断与 http_get 同款意识（坑五： 全文回填一轮撑爆上下文）。
 */
public class FileTools {
  ...
  public String writeFile(
    ...
    // Sandbox 检查位：24 节接 sandbox.enforce(new SandboxAction(FILE_WRITE, path))
    try {
      Path file = Path.of(path);
  ...
  public String listDir(@ToolParam(description = "要列出的目录路径") String path) {
    // Sandbox 检查位：24 节接 sandbox.enforce(new SandboxAction(FILE_READ, path))
