...
**第一阶段已交付（v0.1.0）**：单机运行时内核完整落地——以被验证过的参照实现为基准逐节交付、等价验收，两个日跑 Demo Agent 在真实环境按点自跑，打包为一个 fat JAR，见[路线图](./roadmap)与逐节规格及验收证据（[`specs/`](https://github.com/XianReallyHot-ZZH/YokeOS/tree/master/specs)）。文档页描述的即是已交付形态。
