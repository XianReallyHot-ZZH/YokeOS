package com.yokeos.core.provider;

/**
 * 模型提出的单个工具调用请求（契约上移，第 17 节拍板①；31 节补 id）。
 *
 * <p>name 是 Function Calling 的函数名；argumentsJson 是参数的 JSON 文本——解析责任在 ToolExecutor （先解析后执行，坏 JSON
 * 走失败路径），本对象只承载原文。id 是协议层的 tool_call_id（31 节结构化回传： assistant.tool_calls 与 tool
 * 结果靠它配对，拉平文本时代被丢弃——复读方差的根因修复）；本地测试自造可无 id（null）。
 */
public record ToolCallRequest(String id, String name, String argumentsJson) {}
