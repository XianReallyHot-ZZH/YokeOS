"""每日订单对账的确定性抓数脚本（纯标准库、无外部依赖、无 key）。

进上下文的是本脚本输出的差异 JSON，代码本身不进上下文（技 §11.1 附属资源按需取用）。
两份输入由环境变量指向：RECON_ORDERS_CSV（交易库昨日导出）、RECON_SETTLE_CSV（清算库昨日导出）。
"""
...
if not orders_path or not settle_path:
    json.dump(
        {"error": "请设置 RECON_ORDERS_CSV 与 RECON_SETTLE_CSV 指向昨日两库导出"},
        sys.stdout,
        ensure_ascii=False,
    )
    sys.exit(0)
